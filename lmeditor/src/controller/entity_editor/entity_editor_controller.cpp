...
namespace lmeditor
{
entity_editor_controller::entity_editor_controller(
  entt::registry &map,
...
bool entity_editor_controller::handle(lmtk::input_event const &event)
{
    return std::visit(
      lm::variant_visitor{
        [&](
...
entt::entity entity_editor_controller::add_cube(
  entt::registry &map,
...
std::string entity_editor_controller::get_unique_name(
...
entt::entity entity_editor_controller::add_adjacent(
  entt::registry &map,
...
entt::entity entity_editor_controller::copy_entity(
  entt::registry &map,
  Eigen::Vector3f const &direction)
{
...
void entity_editor_controller::translate(
  entt::registry &map,
  entt::entity entity,
...
std::vector<command_description>
  entity_editor_controller::get_command_descriptions()
{
...
std::vector<command_description>
  entity_editor_controller::get_command_descriptions(
    entity_editor_controller::command_list const &list,
...
Eigen::Vector3f
  entity_editor_controller::view_to_axis(Eigen::Vector3f const &view_vector)
{
    return lm::snap_to_axis(view_to_world(view_vector));
}

Eigen::Vector3f entity_editor_controller::get_selection_extents(entt::registry const &map) const
{
    auto selected_box = get_selection();
...
void entity_editor_controller::select(entt::entity entity)
{
    clear_selection();
    map->emplace<selected>(entity);
}

void entity_editor_controller::clear_selection() const { map->clear<selected>(); }

entt::registry *entity_editor_controller::get_registry() const { return map; }

bool entity_editor_controller::have_selection() const
{
    return !map->view<selected>().empty();
}

entt::entity entity_editor_controller::get_selection() const
{
    return lmeditor::get_selection(*map);
}

lm::camera entity_editor_controller::get_camera() const { return camera; }
...
Eigen::Vector3f entity_editor_controller::get_selection_position(const entt::registry &map) const
{
